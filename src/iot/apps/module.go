...
func (a *App) NewModuleFromFile(filename string, parent *Module) (*Module, error) {
	if absfile, err := filepath.Abs(filename); err != nil {
		return nil, err
	} else if m, ok := a.moduleCache[absfile]; ok {
		return m, nil
		...
		if module, err := a.context.Create(m); err != nil {
			...
			if require, err := m.newRequire(); err != nil {
				return nil, err
			} else if filename, err := a.context.Create(absfile); err != nil {
				return nil, err
...
func (m *Module) Resolve(in v8.FunctionArgs) (*v8.Value, error) {
	...
	} else if !path.IsAbs(abs) {
		for _, p := range m.Paths {
			t := path.Join(p, abs)
			if fi, err := os.Lstat(t); err == nil && fi.Mode().IsDir() {
				abs = t
				break
			}
	...
	if fi, err := os.Lstat(abs); err == nil && fi.Mode().IsDir() {
		abs = path.Join(abs, "index.js")
	}

	// check root of path is rooted inside sandbox

	if root, err := filepath.Abs(m.App.Name); err != nil {
		return nil, err
	} else if !strings.HasPrefix(abs, root+"/") {
		return nil, fmt.Errorf(`cannot find module "%s"`, dst)
	}
...
func (m *Module) newPaths() ([]string, error) {
	...
	if root, err := filepath.Abs(m.App.Name); err != nil {
		...
		for _, p := range strings.Split(m.Dirname, string(filepath.Separator)) {
